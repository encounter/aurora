...
size_t CardRawFile::extract_gci(const char* filename, void* output, const size_t capacity) {
  if (filename == nullptr) {
    return 0;
  }
  FileHandle handle;
...
bool CardRawFile::insert_gci(const void* data, const size_t size, const bool replace) {
