...
TEST_F(TimeTest, ScalesGuestTimeWithoutScalingNativeTime) {
  s_now += 1s;
  EXPECT_EQ(aurora::time::guest_clock::now().time_since_epoch(), 1s);
  EXPECT_EQ(aurora::time::native_clock::now().time_since_epoch(), 1s);

  aurora::time::set_scale(4.0f);
  s_now += 2s;
  EXPECT_EQ(aurora::time::guest_clock::now().time_since_epoch(), 9s);
...
TEST_F(TimeTest, ScaleChangesPreserveContinuity) {
  s_now += 2s;
  aurora::time::set_scale(4.0f);
  EXPECT_EQ(aurora::time::guest_clock::now().time_since_epoch(), 2s);

  s_now += 1s;
  aurora::time::set_scale(0.5f);
  EXPECT_EQ(aurora::time::guest_clock::now().time_since_epoch(), 6s);

  s_now += 2s;
  EXPECT_EQ(aurora::time::guest_clock::now().time_since_epoch(), 7s);
}
...
TEST_F(TimeTest, ExplicitAndLifecyclePausesAreIndependent) {
  ...
  s_now += 3s;
  EXPECT_EQ(aurora::time::guest_clock::now().time_since_epoch(), 0s);
